...
function drawHistogram(){
  ...
  // d3.csv("master.csv", function(error, masterData) {
    d3.csv("genre_mapping.csv", function(error, genreMapping) {
      d3.csv("character_list5.csv", function(error, characterData) {
        d3.csv("meta_data7.csv", function(error, metaData) {
          d3.csv("disney_films2.csv", function(error, disneyFilms) {
    ...
    for (var gender in ageNest){
      var genderSum = 0;
      var ageValues = [];
      for (var age in ageNest[gender].values){
        ageNest[gender].values[age].genderTag = ageNest[gender].key
        if (+ageNest[gender].values[age].key > 0 && +ageNest[gender].values[age].key < 110 && +ageNest[gender].values[age].key != ""){
          ageValues.push(ageNest[gender].values[age])
          // if(ageNest[gender].key == "f"){
          //   console.log(ageNest[gender].values[age].values/4404524);
          //   sum = sum + +ageNest[gender].values[age].values
          // }

          genderSum = ageNest[gender].values[age].values + +genderSum;
        }
      }
      ageNest[gender].values = ageValues;
      ageNest[gender].total_words = genderSum;
      if(ageNest[gender].key != "?"){
        ageData.push(ageNest[gender]);
      }
    }
    ...
    var scriptNest = d3.nest().key(function(d) {
      ...
      .rollup(function(leaves) {
        var female_words = d3.sum(leaves, function(d) {
          if(d.gender == "f"){
            return d.words;
          }
        });
        ...
        // }
        if(mobile){
          var femalePercent = Math.round(100*female_words/total_words)/100;
          if (femalePercent*100 % 2 != 0){
            femalePercent = Math.round(femalePercent*100);
            if(femalePercent % 2 != 0){
              femalePercent = femalePercent + 1;
            }
            femalePercent = femalePercent/100;
          }
          return {"female_percent":femalePercent,"female_words":female_words,"total_words":total_words};
        }
        return {"female_percent":Math.round(100*female_words/total_words)/100, "female_words":female_words,"total_words":total_words};
      })
      .entries(characterData);
    ...
    scriptNest = scriptNest.map(function(d){
      ...
      return {"lines_data":metaData.lines_data,"gross":metaData.gross,"imdb_id":metaData.imdb_id,"title":metaData.title,"year":metaData.year,"female_percent":d.values.female_percent, "female_words":d.values.female_words,"total_words":d.values.total_words,"script_id":+d.key};
    })
    ;
    ...
    var values = [];
    var femaleColor = "#F33" // var femaleColor = "#E0451F" femaleColor = "#FA0E4F"
    var maleColor = "#2161FA" //var maleColor = "#2161FA" #18638B var maleColor = "#249EFB" //var maleColor = "#2161FA"

    movieData = scriptNest;

    var formatCount = d3.format(".0%");
    var interpolateOne = d3.interpolate(maleColor,"#ddd");
    var interpolateTwo = d3.interpolate("#ddd",femaleColor);

    var colorScale = d3.scale.threshold().domain([0.1,0.4,0.6,0.9,1]).range([maleColor,interpolateOne(0.5),"#ddd",interpolateTwo(0.5), femaleColor]);
    var colorScaleTwo = d3.scale.threshold().domain([0.1,0.4,0.6,0.9,1]).range([maleColor,interpolateOne(0.5),"#5D5A5A",interpolateTwo(0.5), femaleColor]);

    var colorScaleContinuous = d3.scale.linear().domain([0,0.25,0.5,1]).range(["#2161FA","#249EFB","#ddd",femaleColor]);
    ...
    for (item in movieData){
      var femalePercentBucket = movieData[item].female_percent;
      ...
      movieData[item].yOrder = femalePercentOrder[femalePercentBucket];
    ...
    function spectrumChart(){
      ...
      var disneyScaleFemale = d3.scale.linear().domain([0,1]).range(["#CCC",femaleColor]);
      var disneyScaleMale = d3.scale.linear().domain([0,1]).range(["#CCC",maleColor]);
      ...
      for (bucket in bucketElements){
        ...
        if(mobile == 1 && bucket == 0){
          ...
          statContainer.append("div")
            .attr("class","film-female-stat tk-futura-pt")
            .style("color",function(d){
              if (d.female_percent < 0.333333){
                return maleColor;
              }
              else if (d.female_percent > 0.666665){
                return femaleColor;
              }
            })
            .html(function(d,i){
              if (i==0 && d.female_percent < .33){
                return percentFormat(1-d.female_percent) + " Male Dialogue"
              }
              else if (i==0 && d.female_percent > .6666665){
                return percentFormat(d.female_percent) + " Female Dialogue"
              }
              else if (i % 5 == 0 && d.female_percent < .33){
                return percentFormat(1-d.female_percent)
              }
              else if (i % 5 == 0 && d.female_percent > .66666665){
                return percentFormat(d.female_percent)
              }
              else if (i % 5 == 0){
                return "<span class='male-color'>" + percentFormat(1-d.female_percent) + "</span>/<span class='female-color'>" + percentFormat(d.female_percent) + "</span>"
              }
              return null;
            })
            .style("line-height",function(d,i){
              if (i==0 && (d.female_percent < .33 || d.female_percent > .6666665)){
                return "10px";
              }
              return null;
            })
          ...
          disneyAxisBucket.append("div")
            ...
            .text(function(d){
              if(bucketElements[bucket] == 1){
                return "Men have 60%+ Dialogue"
              }
              else if (bucketElements[bucket] == 2){
                return "Gender Balance, +/- 10%"
              }
              return "Women have 60%+ Dialogue"
            })
            ;
        }
      ...
      function stageTwo(){
        ...
        for (item in filteredSpectrumData){
          var femalePercentBucket = filteredSpectrumData[item].female_percent;
          if(femalePercentBucket in femalePercentOrder){
            femalePercentOrder[femalePercentBucket] = femalePercentOrder[femalePercentBucket] + 1
          }
          else{
          ...
          filteredSpectrumData[item].yOrder = femalePercentOrder[femalePercentBucket];
          ;
        }
        ...
        var filmElementsNew = filmElements
          ...
          .style("left",function(d){
            if(mobile){
              return Math.round(cx(d.female_percent)) + "px";
            }
            return cx(d.female_percent) + "px";
          })
          .attr("class","film-element")
        ...
        femaleTop.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",axisWidth - 1 +"px");
        maleTop.transition().duration(durationAmount).delay(delayAmount).style("opacity",1);
        if(mobile){
          maleForty.style("opacity",0);
          femaleSixty.style("opacity",0);
          maleTen.style("opacity",0);
        }else{
          maleForty.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",axisWidth*(0.4)+"px");
          femaleSixty.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",axisWidth*(0.6)+"px");
          maleTen.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",axisWidth*(0.1)+"px");
        }
        maleTwentyFive.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",axisWidth*(0.25)+"px");
          ...
          .on("enter", function (e) {
            if(e.target.controller().info("scrollDirection") == "FORWARD"){
              ...
              if(stage == 1){
                ...
                femaleTop.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",900 - 1 +"px");
                maleTop.transition().duration(durationAmount).delay(delayAmount).style("opacity",1);
                maleForty.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",900*(0.4)+"px");
                maleTen.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",900*(0.1)+"px");
                maleTwentyFive.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",900*(0.25)+"px");
                femaleSixty.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",900*(0.6)+"px");
      ...
      function updateSpectrumSearch(data){
        ...
        filmElements
          ...
          .style("background-color",function(d){
            ...
            return d3.rgb(colorScaleContinuous(d.female_percent));
          })
          .transition()
          .duration(500)
          .delay(function(d,i){
        ...
        if(stage == 3){
          ...
          function axisSize(size){
            return filmElements.filter(function(d){
              return d.female_percent < size;
            }).size();
          }

          markerFixed.transition().duration(250).style("left",axisSize(data.female_percent)+"px");
          previewNameContainerFixed.transition().duration(250).style("left",axisSize(data.female_percent)+"px");
          ...
          previewData.data([data.female_percent,1-data.female_percent]).enter();
      ...
      function adjustAxis(genre){
        ...
        //
        if (genre == "all"){
          femaleTop.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",filmElements.size() - 1 +"px");
          maleTop.transition().duration(durationAmount).delay(delayAmount).style("opacity",1);
          maleForty.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",axisSize(0.4)+"px");;
          maleTen.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",axisSize(0.1)+"px");;
          maleTwentyFive.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",axisSize(0.25)+"px");;
          femaleSixty.transition().duration(durationAmount).delay(delayAmount).style("opacity",1).style("left",axisSize(0.6)+"px");;
          ...
          femaleTop.transition().duration(100).style("opacity",0);
          maleTop.transition().duration(100).style("opacity",0);
          maleForty.transition().duration(100).style("opacity",0);
          maleTen.transition().duration(100).style("opacity",0);
          maleTwentyFive.transition().duration(100).style("opacity",0);
          femaleSixty.transition().duration(100).style("opacity",0);
      ...
      var maleTop = histogramTwoAxis.append("div")
        .style("left","0%")
        .attr("class","histogram-two-axis-tick")
        .style("border-color",maleColor)

      var maleTopText = maleTop
        .append("p")
        .style("color",maleColor)
        .style("left","-1px")
        .style("width","100px")
        .attr("class","histogram-two-axis-text tk-futura-pt")
        .html("100% of Words<br>are Male")
        ;

      var femaleTop = histogramTwoAxis.append("div")
        .style("left",axisWidth - 1 +"px")
        .attr("class","histogram-two-axis-tick")
        .style("border-color",femaleColor)
        ;
      ...
      var femaleTopText = femaleTop
        ...
        .attr("class","histogram-two-axis-text tk-futura-pt")
        .html("100% of Words<br>are Female")
        ;

      var maleTen = histogramTwoAxis.append("div")
        .style("left",axisWidth*(0.1)+"px")
        .attr("class","histogram-two-axis-tick")
        .style("border-color",maleColor)

      var maleTenText = maleTen
        .append("p")
        .style("color",maleColor)
        .attr("class","histogram-two-axis-text tk-futura-pt")
        .html("<br>90%")
      ...
      var maleTwentyFive = histogramTwoAxis.append("div")
        .style("left",axisWidth*(0.25)+"px")
        .attr("class","histogram-two-axis-tick")
        .style("border-color",maleColor)

      var maleTwentyFiveText = maleTwentyFive
        .append("p")
        .style("color",maleColor)
        .attr("class","histogram-two-axis-text tk-futura-pt")
        .html("<br>75%")
      ...
      var maleForty = histogramTwoAxis.append("div")
        .style("left",axisWidth*(0.40)+"px")
        .attr("class","histogram-two-axis-tick")
        .style("border-color",maleColor)

      var maleFortyText = maleForty
        .append("p")
        .style("color",maleColor)
        .attr("class","histogram-two-axis-text tk-futura-pt")
        .html("<br>60%")
      ...
      var midPointText = midPoint
        ...
        .html("50/50<br>Gender Split")
        ;

      var femaleSixty = histogramTwoAxis.append("div")
        .style("left",axisWidth*(0.60)+"px")
        .attr("class","histogram-two-axis-tick")
        .style("border-color",femaleColor)

      var femaleSixtyText = femaleSixty
        .append("p")
        .style("color",femaleColor)
        .attr("class","histogram-two-axis-text tk-futura-pt")
        .html("<br>60%")
      ...
      function changeMovieLines(data){
        ...
        scriptLinesPercentContainer.append("p")
          .html(function(d){
            var percent = "<span style='color:"+maleColor+";'>"+percentFormat(1-data.female_percent)+" Male</span>/<span style='color:"+femaleColor+";'>"+percentFormat(data.female_percent)+" Female</span>";
            return percent;
          })
        ...
        // var valuesLines = [];
        // var linesScale = d3.scale.quantize().domain([.6,1]).range(["m","f"])
        ...
        var scriptLineFemaleScale = d3.scale.linear().domain([0,14]).range(["#CCC",femaleColor]);
        var scriptLineMaleScale = d3.scale.linear().domain([0,14]).range(["#CCC",maleColor]);
        var barHeight = 50;
        ...
        scriptLinesDataLines.selectAll("div")
          ...
          .style("background-color",function(d,i){
            if(i == 1){
              return scriptLineFemaleScale(d);
            }
            return scriptLineMaleScale(d);
          })
        ...
        scriptLines
          .append("p")
          .attr("class","histogram-two-script-data-label histogram-two-script-data-label-women tk-futura-pt")
          .style("color",femaleColor)
          .text("Female Lines")
          ;
        ...
        scriptLines
          ...
          .text(function(d){
            return "male/female Lines by minute"
          })
          ;
    ...
    //     ;
    //
    //     // var femaleColor = "#E0451F"
    //     var femaleColor = "#E0451F"
    //     //var maleColor = "#249EFB" //var maleColor = "#2161FA"
    //     var maleColor = "#18638B" //var maleColor = "#2161FA" //18638B
    //
    //
    //   // var maleColor = "#18638B"
    //   // var femaleColor = "#E0451F"
    //
    //   var disneyScaleFemale = d3.scale.linear().domain([0,1]).range(["#CCC",femaleColor]);
    //   var disneyScaleMale = d3.scale.linear().domain([0,1]).range(["#CCC",maleColor]);
    //
    //   // var disneyScaleFemale = d3.scale.linear().domain([0,.5,1]).range(["#FFF","#CCC",femaleColor]);
    //   // var disneyScaleMale = d3.scale.linear().domain([0,.5,1]).range(["#FFF","#CCC","#2161FA"]);
    //
    //   // var disneyScaleFemale = d3.scale.linear().domain([0,1]).range(["#CCC",femaleColor]);
    //   // var disneyScaleMale = d3.scale.linear().domain([0,1]).range(["#CCC","#2161FA"]);
    //
    //
    //   var randomScale = d3.scale.linear().domain([0,1]).range([0,movieData.length]);
    ...
    //     element.disneyBucket = bucketAssign(element.female_percent)
    ...
    //       if (a.female_percent < 0.333){
    //         return ((a.female_percent < b.female_percent) ? -1 : 1);
    //         return 0;
    //       }
    //       else {
    //         return ((a.female_percent > b.female_percent) ? -1 : 1);
    ...
    //
    //   statContainer.append("div")
    //     .attr("class","disney-female-stat tk-futura-pt")
    //     .style("color",function(d){
    //       if (d.female_percent < 0.3333){
    //         return maleColor;
    //       }
    //       else if (d.female_percent > 0.666665){
    //         return femaleColor;
    ...
    //     .html(function(d,i){
    //       if (i==0 && d.female_percent < .33){
    //         return percentFormat(1-d.female_percent) + " Male Lines"
    //       }
    //       else if (i==0 && d.female_percent > .6666665){
    //         return percentFormat(d.female_percent) + " Female Lines"
    //       }
    //       else if (i % 5 == 0 && d.female_percent < .33){
    //         return percentFormat(1-d.female_percent)
    //       }
    ...
    function histogramThree(){
      ...
      var movieMap = movieData.map(function(d){ return d.female_percent; })

      var middlePercentile = movieData.filter(function(d){
        return d.female_percent<0.5;
      }).length;
      ...
      d3.select(".histogram-three")
        ...
        .html("Sorted by Films with <span class='histogram-three-highlight'>90%+</span> Male Lines ")
      ...
      d3.select(".histogram-three-spectrum")
        .append("p")
        .attr("class","histogram-three-spectrum-male tk-futura-pt")
        .style("color",maleColor)
        .text("More Lines from Men")
        ;

      d3.select(".histogram-three-spectrum")
        .append("p")
        .attr("class","histogram-three-spectrum-female tk-futura-pt")
        .style("color",femaleColor)
        .text("More Lines from Women")
        ;
      ...
      var histogramThreeDot = histogramThreeRow.append("div")
        ...
        .style("left",function(d){
          return (d.female_percent)*100+ "%";
        })
        .style("background-color", function(d){
          return colorScaleContinuous(d.female_percent);
        })
        ;
    ...
    function allFilms(){
      ...
      var bucketText = ["90%+<br>Male","60% - 90%<br>Male","Gender Parity<br>+/- 10%","60% - 90%<br>Female","90%+<br>Female"];
      ...
      var filteredBucketData = movieData.filter(function(d,i){
        ...
        return bucketScale(d.female_percent) == bucketSelected;
      })
      ;
      ...
      function detailElementsDraw(){
          ...
          .on("mouseover",function(d,i){
            if(firstHover){
              ...
              detailChartData
                ...
                .style("background-color",function(d,i){
                  var filmFemalePercent = d3.select(this.parentNode).datum().female_percent;
                  if(d.gender=="m"){
                    return detailScaleMale(1-filmFemalePercent);
                  }
                  return detailScaleFemale(filmFemalePercent);
                })
                .style("left",function(d){
                  var totalWords = d3.select(this.parentNode).datum().total_words;
        ...
        var detailScaleFemale = d3.scale.linear().domain([0,1]).range(["#CCC",femaleColor]);
        var detailScaleMale = d3.scale.linear().domain([0,1]).range(["#CCC",maleColor]);
      ...
      function distributionUpdate(){
        ...
        var movieMap = filteredBucketData.map(function(d){ return d.female_percent; })
      ...
      function detailChartUpdate(){
        ...
        filteredBucketData = movieData.filter(function(d,i){
          ...
          if (genreSelectedAll == "all" && eraSelected == "all"){
            ...
            return bucketScale(d.female_percent) == bucketSelected && d.year.slice(2,3) == eraSelected;
          }
          else if(genreSelectedAll != "all" && eraSelected == "all"){
            return bucketScale(d.female_percent) == bucketSelected && string.indexOf(genreSelectedAll) > -1;
          }
          return bucketScale(d.female_percent) == bucketSelected && string.indexOf(genreSelectedAll) > -1 && d.year.slice(2,3) == eraSelected;
        })
        ;
    ...
    function ageHistogram(){
      ...
      for (var item in ageData){
        var gender = ageData[item].key;
        var genderTotal = ageData[item].total_words;
        ageMax[gender] = genderTotal;
      }
      ...
      for (var genderBucket in ageNest){
        var gender = ageNest[genderBucket].key;
        if (ageNest[genderBucket].key != "?"){
          var buckets = d3.layout.histogram()
          ...
          ageContainer.append("p")
            ...
            .text(function(d){
              if(gender == "m"){
                return "Actors: Total Words, by Age";
              }
              return "Actresses: Total Words, by Age";
            })
          ...
          var agePercent = ageDataHistogram
            ...
            .text(function(d,i){
              ...
              if(i==0){
                return percentFormat(d.total/totalSum) + " of All "+textGender+" Dialogue in Dataset"
              }
      ...
      var area = d3.svg.area()
        ...
        .y0(height)
        .y1(function(d) { return y(d.values/ageMax[d.genderTag]); });
      ...
      ageChart.append("g").selectAll("path")
          ...
          .attr("fill", function(d,i){
            if(d.key == "f"){
              var rgbColor = d3.rgb(femaleColor)
              return "rgba("+rgbColor.r+","+rgbColor.g+","+rgbColor.b+",.3)";
            }
            var rgbColor = d3.rgb(maleColor)
    ...
    //
    // console.log(d3.mean(romComArray,function(d,i){return d.female_percent;}));
    ...
    //   femaleCount = 0;
    //
    //   // if(characterNest[script].values[characterNest[script].values.length-1].gender == "m"){
    //   //   maleLead = maleLead + 1;
    //   // }
    //   if(characterNest[script].values[characterNest[script].values.length-1].gender == "f"){
    //     femaleCount = femaleCount + 1;
    //   }
    //     if(characterNest[script].values[characterNest[script].values.length-2].gender == "f"){
    //       femaleCount = femaleCount + 1;
    //     }
    //     // if(characterNest[script].values[characterNest[script].values.length-2].gender == "m"){
    //     //   maleSupport = maleSupport + 1;
    //     // }
    //     if(characterNest[script].values.length > 2){
    //       if(characterNest[script].values[characterNest[script].values.length-3].gender == "f"){
    //         femaleCount = femaleCount + 1;
    //       }
    //       // if(characterNest[script].values[characterNest[script].values.length-3].gender == "m"){
    //       //   maleSupport = maleSupport + 1;
    //       // }
    //     }
    //     if (femaleCount > 1){
    //       femaleSupport = femaleSupport + 1;
    //     }
    // }
  ...
  //   for (line in array){
  //     var row = {line_index:array[line],gender:lineByLine[item].actor_gender, script_id:lineByLine[item].script_id};
  //     lineArray.push(row);
  //   }
  // }
  // //
  ...
  //
  //       if (minute in lineBreakdown){
  //         if(lineNest[script].values[line].gender == "m"){
  //           lineBreakdown[minute][0] = lineBreakdown[minute][0] + 1;
  //         }
  //         else{
  //           lineBreakdown[minute][1] = lineBreakdown[minute][1] + 1
